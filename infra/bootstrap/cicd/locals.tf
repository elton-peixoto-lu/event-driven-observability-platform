locals {
  ...
  apply_allowed_subjects = flatten([for repo in local.allowed_repo_slugs : ["repo:${repo}:ref:refs/heads/${var.default_branch}", "repo:${repo}:environment:${var.github_apply_environment_name}"]])
